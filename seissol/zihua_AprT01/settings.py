...
surrogate_model_settings = surrogate_model.SKLearnGPSettings(
    ...
    correlation_kernel=RBF(length_scale=4 * (0.1,), length_scale_bounds=4 * ((1e-5, 1e5),)),
    data_noise=1e-6,
    ...
    init_seed=1,
    checkpoint_load_file="ridgecrest_4D_preT110/surrogateDiv2_checkpoint_Bpretraining.pkl",
    # checkpoint_load_file=None,
    checkpoint_save_path=Path("ridgecrest_AprT01_T1"),
)
...
control_logger_settings = utils.LoggerSettings(
    do_printing=True,
    logfile_path=Path("ridgecrest_AprT01_T1/online.log"),
    write_mode="w",
)
...
pretraining_settings = offline_training.OfflineTrainingSettings(
    ...
    lhs_bounds=[[0.1, 2.0], [0.1, 2.0], [0.4, 1.4], [0.4, 1.4]],
    lhs_seed=111,
...
pretraining_logger_settings = utils.LoggerSettings(
    do_printing=True,
    logfile_path=Path("ridgecrest_4DT01_preT222/pretraining.log"),
    write_mode="w",
)
...
test_client_settings = test_client.TestClientSettings(
    ...
    #training_params=np.random.uniform(0.5, 0.9, (5, 4)),
    training_params=np.random.uniform(np.array([0.5,0.5,0.7,1.1]), np.array([0.7,0.7,0.9,1.3]), size=(5,4))
)

# --------------------------------------------------------------------------------------------------
visualization_settings = visualization.VisualizationSettings(
    offline_checkpoint_file=Path("ridgecrest_4D_preT110/surrogateDiv2_checkpoint_Bpretraining.pkl"),
    online_checkpoint_filestub=Path("ridgecrest_AprT01_T1/surrogate_checkpoint"),
    #online_checkpoint_filestub=None,
    visualization_file=Path("ridgecrest_AprT01_T1/visualization.pdf"),
    # visualization_points=np.column_stack(
    #     (np.repeat(np.linspace(0.5, 2.5, 100), 100), np.tile(np.linspace(0.3, 0.9, 100), 100))
    # ),
    visualization_bounds=[[0.1,2.0],[0.1,2.0],[0.4,1.4],[0.4,1.4]]
)
